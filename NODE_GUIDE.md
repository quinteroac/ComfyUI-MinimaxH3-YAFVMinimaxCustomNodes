...
El modo automático usa como objetivo unos 22 fotogramas de solapamiento y redondea el tamaño a la cuadrícula de H3 (5 + 17k), de forma que el número solicitado de ventanas cubra el vídeo completo. Los inicios mantienen la fase temporal del latente; la última ventana puede tener más solapamiento para cubrir el final sin truncar el vídeo.

Cada ventana ejecuta todo el tramo de Pass 2 (`split_step → total_steps`). La ejecución es serial: cada chunk posterior recibe un prefijo sintético de cinco fotogramas, coloca la cola anterior en posiciones target que sí se denoisean y la entrega además como referencia nativa `Video1/Audio1` junto con una keyframe visual corta. Al ensamblar se descarta sólo el prefijo sintético; las posiciones warm-start se conservan una sola vez. No se mezclan predicciones ni se aplica crossfade.

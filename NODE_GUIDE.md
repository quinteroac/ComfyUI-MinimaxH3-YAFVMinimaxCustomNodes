...
Cada ventana ejecuta todo el tramo de Pass 2 (`split_step → total_steps`). El ruido inicial procede de un único campo compartido en CPU; el ruido adicional de los samplers estocásticos sigue su comportamiento nativo dentro de cada ventana. Se mezclan los resultados con rampas normalizadas en los solapamientos, y el latente final conserva resolución y duración. No se mezclan predicciones entre ventanas en cada paso del sampler.

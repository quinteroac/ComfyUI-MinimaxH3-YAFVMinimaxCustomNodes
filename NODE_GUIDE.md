...
Cada ventana ejecuta todo el tramo de Pass 2 (`split_step → total_steps`) a partir del latente de Pass 1, después del upscale si está activado, con el mismo ruido en las posiciones compartidas. Ninguna ventana recibe el resultado de otra ni congela su solapamiento. Las predicciones se combinan mediante rampas normalizadas en el solapamiento, sin desplazar posiciones temporales. Con chunks de 56/22, las ventanas avanzan 34 fotogramas; un vídeo de 362 fotogramas utiliza 10 ventanas. La mezcla suaviza las uniones, pero puede producir ghosting si las predicciones divergen; la calidad requiere validación con el checkpoint real.

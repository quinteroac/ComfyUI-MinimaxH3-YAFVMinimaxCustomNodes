...
def sample_temporal(model, seed, steps, cfg, sampler_name, scheduler, positive,
    ...
    windows = temporal_windows(video.shape[2], chunk_frames, overlap_frames, exact_tail=chunk_count is not None)
    ...
    if mask is not None:
        mask = mask.unbind()[0] if mask.is_nested else mask
        mask = comfy.utils.reshape_mask(mask.cpu().float(), video.shape)

    accumulated = torch.zeros_like(video, dtype=torch.float32, device="cpu")
    weights = torch.zeros((1, 1, video.shape[2], 1, 1), dtype=torch.float32, device="cpu")
    ...
    for index, (start, stop) in enumerate(windows):
        ...
        window = NestedTensor((video[:, :, start:stop].clone(), audio[..., a0:a1].clone()))
        window_noise = NestedTensor((noise_video[:, :, start:stop].clone(), noise_audio[..., a0:a1].clone()))
        video_mask = (mask[:, :, start:stop].clone() if mask is not None
                      else torch.ones((1, 1, stop - start, 1, 1), dtype=torch.float32))
        window_mask = NestedTensor((video_mask, torch.zeros((1, 1, 2, a1 - a0), dtype=torch.float32)))
        ...
        # The overlap provides temporal context to both samplers, but averaging
        # two independently generated latents across the whole overlap can
        # create a visible double image. Keep one owner for most positions and
        # use only a very short crossfade at the ownership boundary.
        fade = torch.ones(stop - start, dtype=torch.float32)
        if index > 0:
            overlap = windows[index - 1][1] - start
            blend = min(2, overlap)
            cut = (overlap + 1) // 2
            ramp_start = max(0, cut - blend // 2)
            incoming = torch.ones(overlap, dtype=torch.float32)
            incoming[:ramp_start] = 0
            incoming[ramp_start:ramp_start + blend] = torch.arange(1, blend + 1, dtype=torch.float32) / (blend + 1)
            fade[:overlap] = incoming
        if index + 1 < len(windows):
            ...
            incoming[:ramp_start] = 0
            incoming[ramp_start:ramp_start + blend] = torch.arange(1, blend + 1, dtype=torch.float32) / (blend + 1)
            incoming[ramp_start + blend:] = 1
            fade[-overlap:] = 1 - incoming
        fade = fade.view(1, 1, -1, 1, 1)
        accumulated[:, :, start:stop].add_(refined * fade)
        weights[:, :, start:stop].add_(fade)
        del sampled, refined, window, window_noise, window_mask, cond, uncond
        mm.throw_exception_if_processing_interrupted()

    accumulated.div_(weights)
    result = dict(latent)
    result["samples"] = NestedTensor((accumulated.to(video.dtype), audio))
    return result
